...
def test_create_login(tmpdir):
    ...
    assert len(backend.entries.items) == 1
    # valid logins
    backend.create_login(u"test_name", "bla@gmail.com", "geheim", "https://www.google.com/login")
    # 2 accounts, same domain
    backend.create_login(u"test_name", "blubb@gmx.net", "geheim2", "https://gmx.net/login")
    backend.create_login(u"test_name", "blubb2@gmx.net", "geheim3", "https://gmx.net/login")
    # another login
    backend.create_login(u"test_name", "asdasd@web.de", "geheim4", "http://web.de/login/form.php")
